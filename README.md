...
- **OBS 오버레이 구조 분리 강화**: 곡 정보(`overlay-info`)와 가사(`overlay-lyrics`) 오버레이를 분리 운용하고, 미리보기 탭/URL 가이드를 기준 크기와 권장 크기에 맞춰 정비.
- **곡 정보 오버레이 안정화**: 긴 제목 마키 계산을 썸네일/패딩/갭 포함 실측 폭 기준으로 보정하고, 기본 썸네일 fallback 및 트랙 전환 시 hide→show 시퀀스 적용.
- **가사 오버레이 표시 타이밍 개선**: 재생 시작 직후 첫 가사가 하단 대기열에 즉시 보이고, 타임코드 진입 시 상단 라인으로 승격되도록 동기화 로직 개선.
- **오버레이 전환 일관성 확보**: 곡 전환 시 곡 정보/가사 오버레이 모두 기존 내용이 잠깐 숨겨진 뒤 새 메타데이터로 재표시되도록 통일.
