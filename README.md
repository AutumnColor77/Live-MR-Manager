...
- **통합 6컬럼 그리드 시스템 (v0.5.0)**: 리스트 모드를 단일 그리드 구조로 개편하여 썸네일, 제목, 장르, 태그, 재생 시간의 **완벽한 수직/수평 정렬** 달성.
...
### 📁 라이브러리 및 가사 정렬 (Library & Syncing)
- **수동 가사 정렬 시스템 (Manual Lyric Alignment)**: [v0.7.0] 탭-투-싱크(Tap-to-Sync) 기반의 정밀 가사 동기화 도구 구현.
    - **가변 파형 렌더링**: 오디오 분석 데이터를 기반으로 실시간 파형을 시각화하여 정밀한 타이밍 포인트 선정 지원.
    - **백엔드 통합 엔진 (In-Progress)**: 브라우저 코덱 제한을 극복하기 위해 Rust 네이티브 오디오 엔진과 연동 중. (※ 현재 일부 환경에서 재생 불안정 이슈가 있으며 지속적인 수정 중)
...
- [X] **실시간 태그 번역 시스템 (Dynamic Translation)**: 백엔드 필터를 통한 장르/태그 실시간 번역 및 사전(Dictionary) 업데이트 즉시 반영 시스템 구축 완료. (v0.6.0)
- [X] **MR 관리 안정화 및 동기화 패치**: MR 삭제 시 발생하던 프리징 현상을 해결하고 상태 동기화 로직을 개선함. (v0.6.1)
- [X] **오디오 UI 고도화 및 태그 확장 (v0.6.2)**: 전역 폰트 크기 통일, 마스터 볼륨 레이아웃 개편 및 50여 종의 신규 장르/태그 매핑 대거 추가. (v0.6.2)
