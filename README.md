...
- **실시간 태그 번역 시스템 (Dynamic Translation)**: 백엔드에서 음원 목록을 불러올 때 커스텀 사전을 참조하여 장르와 태그를 즉시 번역. 데이터베이스를 직접 수정하지 않으면서도 UI에 즉각적인 번역 결과를 반영하며, 사전 업데이트 시 라이브러리 목록에 즉시 동기화됨. (v0.5.1)
