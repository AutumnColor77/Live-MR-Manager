...
- **전역 30px 세이프 에리어 (Safe-area Layout)**: [v0.9.0] 모든 페이지(Library, Lyric Sync, Settings, Task Manager)에 엄격한 좌측 정렬 기준선(30px)을 적용하여 시각적 일관성과 안정성 확보.
...
- **정밀 가사 동기화 도구 (Manual Lyric Sync)**: [v0.9.5] 탭-투-싱크(Tap-to-Sync) 방식의 고성능 Lyric Sync 도구.
...
- [X] **실시간 가사 드로어 (Lyric Drawer)**: [v0.9.5] 음악 재생 시 오른쪽에서 슬라이딩되는 가사 전용 창. 실시간 하이라이트 동기화 및 자동 스크롤 지원.
- [X] **v0.6.2 그리드 카드 디자인 복원**: [v0.9.5] 과거의 정갈한 서식(15px 제목, 파란색 가수명 배지, 대문자 태그)과 역동적인 호버 효과 복원.
...
- [X] **방송 오버레이 커스텀 (Broadcast Integration)**: [v0.10.0] 실시간 방송 송출을 위한 전문적인 오버레이 관리 시스템.
