...
Songbook 채널 안내, 신청목록·사이드바 배지, 홍보 데모 화면을 다듬었습니다. 기준: v0.7.8.

### Songbook

- **채널 만들기**: 앱이 `POST /api/me/channels`로 채널을 만들지 않습니다. 채널이 없으면 확인 후 [내 계정](https://www.livemrsongbook.com/me)을 열고, 브라우저에서 만든 뒤 다시 동기화하라는 안내만 띄웁니다. 취소하면 조용히 닫힙니다.
- **같은 흐름**: 동기화·가져오기, 설정·신청목록의 채널 만들기, 계정 메뉴에서 채널이 없을 때 모두 `/me`로 안내합니다.
- **신청목록**: 이미 로그인한 상태에서 탭을 열 때 로그인 버튼이 먼저 보이지 않습니다. 불러오는 중을 보여 주고, 실패하면 다시 시도만 나옵니다.

### UI

- **AI 프로세싱 배지**: 대기 수가 10 이상이면 배지가 자릿수에 맞게 늘어나고, 숫자가 가운데에 옵니다.

### 홍보 데모 (`#promo`)

- **신청목록**: 데모 채널 대기열(한 곡 재생 중, 나머지 대기)을 보여 줍니다. 조작은 메모리 안에서만 바뀝니다.
- **유튜브 검색**: 예시 곡 **Dynamite BTS**로 실제 검색 결과를 띄웁니다.

### 개발

- **Windows `rustc` 1.94**: `tauri-app`을 전체 디버그 정보로 컴파일하면 컴파일러가 죽는 경우가 있어, 개발 프로필에서 이 크레이트만 줄 번호 디버그 정보(`debug = 1`)를 씁니다.
