...
const modelDescMap = {
  ...
  inst_hq_3: "반주(MR) 품질을 우선할 때 추천합니다.",
};
...
export async function refreshModelDropdown() {
  ...
  await refreshModelUiState(effectiveActive);
}
...
function isCustomModel(modelId) {
  return modelListCache.some((m) => m.id === modelId && m.isCustom);
}
...
async function refreshModelUiState(modelId) {
  ...
  // Custom models are local-file-only (no download URL, always "ready" once
  // registered), so the download button doesn't apply to them; repurpose the
  // delete button to remove the custom registration instead of the built-in
  // delete-and-redownload flow.
  if (elements.btnDownloadModel) {
    elements.btnDownloadModel.style.display = isCustomModel(modelId) ? "none" : "";
  }
  if (elements.btnDeleteModel) {
    elements.btnDeleteModel.textContent = isCustomModel(modelId) ? "커스텀 모델 제거" : "모델 삭제";
  }
}
  ...
  if (elements.btnDownloadModel) {
    elements.btnDownloadModel.onclick = async () => {
      try {
        ...
        if (isCustomModel(modelId)) return; // 커스텀 모델은 로컬 파일 등록 방식이라 다운로드가 필요 없음
    ...
    elements.btnDeleteModel.onclick = async () => {
      try {
        ...
        if (isCustomModel(modelId)) {
          ...
          import('../../utils.js').then(m => m.showNotification("커스텀 모델이 제거되었습니다.", "info"));
        } else {
