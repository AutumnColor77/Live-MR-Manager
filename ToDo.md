...
- [X] **(버그 수정) AI 분리 중 UI 블로킹 현상**: 분리 프로세스 중에도 다른 곡 재생 등 모든 UI 컨트롤이 가능하도록 비동기 처리 로직 개선 완료.
- [X] **(개선) MR/보컬 트랙 삭제 후 재생**: 분리된 트래(MR 등) 삭제 시, 플레이어가 자동으로 원본 트랙을 재생하도록 즉시 소스 연결 로직 구현 완료.
...
- [ ] **방송 화면용 곡 정보**: OBS 브라우저 소스 등을 위한 실시간 곡 정보 오버레이 (HUD)
- [ ] **방송 화면용 가사**: 실시간 가사 정보를 방송 화면에 최적화하여 송출하는 오버레이
