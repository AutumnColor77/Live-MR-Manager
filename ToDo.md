...
- [X] **OBS 오버레이 2-레이어 운영 고도화 (v0.4.5)**: 곡 정보/가사 오버레이 페이지 분리, 미리보기 탭 및 브라우저 소스 권장 해상도 가이드 정비
- [X] **오버레이 전환 안정화 (v0.4.5)**: 트랙 변경 시 곡 정보/가사 오버레이를 hide→show로 재전환하여 이전 메타데이터 잔상 방지
- [X] **가사 오버레이 타이밍 보정 (v0.4.5)**: 재생 시작 직후 첫 가사 하단 대기 표시 및 타임코드 진입 시 상단 승격 표시 로직 개선
